...
function checkTarget(file, idIndex) {
  if (!fs.existsSync(file)) return [{ file, id: null, ok: false, message: `${file}: target does not exist` }];
  ...
  return ids.map((id) => {
    ...
    if (!source) {
      return { file, id, ok: false, message: `${file}: covers unknown id "${id}" (no document in scope declares it)` };
    }
    if (source.version === null) {
      return { file, id, ok: false, message: `${file}: covers "${id}" (${source.file}), which has no version: field` };
    }
    ...
    return { file, id, ok: true, message: `${file}: covers.${id} matches ${source.file} v${source.version}` };
  });
}

function run(cfg) {
  const targets = (cfg.targets || []).map(toNative);
