...
function run(cfg) {
  ...
  for (const file of files) {
    ...
    checked += 1;
    if (count > max) {
      // O `why` NÃO entra aqui: a CLI já o imprime uma vez no rodapé da regra.
      // Interpolar nos dois lugares enche a saída com o mesmo parágrafo por
      // achado — e saída ruidosa é o começo de ninguém ler a saída.
      findings.push({
        file,
