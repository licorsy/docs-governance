...
function checkPrefixIdPattern(pattern, re, index, file, line, lineIndex, findings) {
  re.lastIndex = 0;
  let m;
    ...
    lines.forEach((line, i) => {
      ...
      for (const pattern of patterns) {
        if (pattern.kind === 'filename') {
          ...
          checkPrefixIdPattern(pattern, prefixRegexes.get(pattern.id), prefixIndexes.get(pattern.id), file, line, i, findings);
        }
      }
    });
  }
