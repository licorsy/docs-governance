...
// rule (declared_counts, sum_decomposition, facts, version_citations).
...
// (ADR-0009: old entries are never rewritten). A line inside a fenced code
// block is an example, not a claim. Exemption is always per LINE (or per
// whole file, for `historical_paths`), never for the entire document — a
// file can have a frozen changelog and a live body in the same breath.
...
// Content there is frozen by the repository's own convention (ADR-0009).
